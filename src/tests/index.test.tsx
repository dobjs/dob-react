...
test("no args with no error and run once", t => {
  ...
  return timeout().then(() => t.true(runCount === 1));
});
...
test("test action and store but use in render!", t => {
  ...
  return timeout().then(() => t.true(runCount === 2)); // 2
});
...
test("innert store connect and global provider", t => {
  ...
  create(
    <Provider
      ...
      globalAction={stores.GlobalAction}
    >
      <App />
    </Provider>
  );
...
test("functional store connect", t => {
  ...
  return timeout().then(() => t.true(runCount === 1)); // 2
});
...
test("unmount will disConnect", t => {
  ...
  return timeout().then(() => t.true(runCount === 2));
});
